/**
 * ProLease IFRS 16 Plugin - Standalone Version for Inline HTML
 *
 * This version uses window globals instead of ES6 imports
 * to work with the standalone HTML where everything is inline
 */

(function(window) {
  'use strict';

  // Wait for DataForge to be ready
  function waitForDataForge(callback) {
    if (window.DJDataForgeKernel && window.DJDataForgeKernel.pluginHost) {
      callback();
    } else {
      setTimeout(() => waitForDataForge(callback), 100);
    }
  }

  // LeaseContract interface
  /**
   * @typedef {Object} LeaseContract
   * @property {string} id
   * @property {string} contractName
   * @property {number} termMonths
   * @property {string} startDate
   * @property {number} totalRent
   * @property {number} serviceDeductions
   * @property {number} discountRate
   * @property {number} initialLandlordAllowance
   * @property {number} initialDirectCosts
   * @property {string} createdAt
   * @property {string} updatedAt
   */

  class ProLeasePluginStandalone {
    constructor() {
      this.manifest = {
        ...
        description: 'IFRS 16 lease accounting calculator (Standalone)',
        permissions: [
          'read:workbook',
          'write:workbook',
          'ui:toolbar',
        ...
        entryPoint: 'prolease-standalone.js',
      };
    ...
    async init(context) {
      ...
      this.log('Initializing v6.0.0-standalone...');
    ...
    registerFormulas() {
      ...
      registry.register(
        ...
        {
          argCount: 3,
          description: 'Calculate present value for lease payments',
        }
      );
      ...
      registry.register(
        ...
        {
          argCount: 1,
          description: 'Convert annual discount rate to monthly rate',
        }
      );
      ...
      registry.register(
        ...
        {
          argCount: 3,
          description: 'Calculate opening ROU asset',
        }
      );
    ...
    renderControlPanel(container) {
      container.innerHTML = `
        <div class="prolease-panel" style="padding: 8px;">
          <button id="prolease-create-btn" style="
            width: 100%;
            padding: 10px;
            margin-bottom: 12px;
            background: #3b82f6;
    ...
    handleNewContract() {
      ...
      this.log('Contract created', contract.id);
    ...
    handleRecalculate(contract) {
      this.log('Recalculating contract', contract.id);
    ...
    async handleDelete(contractId) {
      ...
      if (!confirm(`Delete contract "${contract.contractName}"?`)) return;

      this.contracts = this.contracts.filter((c) => c.id !== contractId);
      await this.saveContracts();

      this.log('Contract deleted', contractId);
    ...
    populateSheet(contract, calculatedRows) {
      try {
        ...
        if (!sheet) {
          ...
          this.log('Created new sheet', sheetName);
        } else {
          ...
          this.log('Cleared existing sheet', sheetName);
        }
        ...
        this.log('Sheet populated', { contract: contract.contractName, rows: dataRows.length });
      ...
      const code = `
        ...
        function getHeaders() {
          return [
            ...
            "C) Landlord TI Allowance after Commence Date", "D) Total Rent to be Capitalized (A + B + C)",
            "Remaining Present Value", "E) Interest", "End of Month Lease Liability",
            "Current Short Term (ST) Liability", "Non Current Long Term (LT) Liability",
            "Proof Column (ST+LT)", "F) New Initial Landlord Allowance", "I) Allowance Amortization",
            "J) Allowance Closing Balance", "K) New IDC", "N) IDC Amortization",
            "O) IDC Closing Balance", "S) Opening ROU Asset", "U) ROU Asset Amortization",
            "V) ROU Asset Closing Balance", "Total ROU Asset Closing Balance (J+O+V)",
            "W) Total P&L Non-Financial Expense", "P&L - Reported Lease Expense (E+W)"
          ];
        }
  ...
  // Export to window
  window.ProLeasePluginStandalone = {
    ProLeasePlugin: ProLeasePluginStandalone,
    manifest: new ProLeasePluginStandalone().manifest,
    version: '6.0.0-standalone',
    name: 'ProLease IFRS 16 Standalone',

    // Auto-register when called
    register: async function() {
      if (!window.DJDataForgeKernel || !window.DJDataForgeKernel.pluginHost) {
        console.error('[ProLeasePlugin] DataForge not loaded. Waiting...');
        return new Promise((resolve) => {
          waitForDataForge(async () => {
            await this._doRegister();
            resolve();
          });
        });
      }
      return await this._doRegister();
    },

    _doRegister: async function() {
      try {
        const plugin = new ProLeasePluginStandalone();
        await window.DJDataForgeKernel.pluginHost.loadPlugin(
          { default: ProLeasePluginStandalone },
          plugin.manifest
        );
        console.log('[ProLeasePlugin] Registered successfully');
        return true;
      } catch (error) {
        console.error('[ProLeasePlugin] Registration failed:', error);
        return false;
      }
    }
  };
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      waitForDataForge(() => {
        window.ProLeasePluginStandalone.register();
      });
    });
  } else {
    waitForDataForge(() => {
