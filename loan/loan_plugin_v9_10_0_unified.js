...
class LoanPluginV971 {
  ...
  _registerMenus(){
    const add = (label, action) => {
      try { this.kernel.registry.registerMenuItem({ pluginId: PLUGIN_ID, label, action }); }
      catch(e){ this.error('menu', e); }
    };
    add('💰 Registrar novo contrato', ()=> this.showNewLoanModal());
    add('📊 Gerar agenda de Accruals', ()=> this._wizardAccrualUnified());
    add('📤 Exportar planilha(s)', ()=> this._wizardExport());
    // Pagamentos dinâmicos: anotação automática ligada no init()
  }
  ...
  // --------- Unified Accrual Wizard ---------
  _wizardAccrualUnified(){
    const wb = this.kernel.workbookManager.getActiveWorkbook();
    const cs = wb?.sheets.find(s=> s.name===LoanPluginV971.S.CONTRACTS);
    if (!cs || cs.rows<=1) return this.ui.showToast('Nenhum contrato.','warning');
    const ids=[]; for (let i=1;i<cs.rows;i++) ids.push(String(cs.getCellValue(i,0)||''));

    const m = this.ui.createModal('📊 Gerar agenda de Accruals');
    m.body.innerHTML = `
      <div style="display:grid;gap:12px">
        <div class="grp"><label>Contrato</label><select id="cid" class="formula-input">${ids.map(x=>`<option>${x}</option>`).join('')}</select></div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
    ...
    f.querySelector('.btn-cancel').addEventListener('click',()=> m.overlay.remove());
    f.querySelector('.btn-ok').addEventListener('click', async()=>{
      const id = String(m.body.querySelector('#cid').value);
      const d0 = String(m.body.querySelector('#d0').value);
      const d1 = String(m.body.querySelector('#d1').value);
      const fq = String(m.body.querySelector('#fq').value);
      const rm = String(m.body.querySelector('#rm').value);
      const fx = String(m.body.querySelector('#fxc').value);
      const out = {
      ...
      await this._generateAccrualsUnified(id, d0, d1, fq, rm, fx, out);
      m.overlay.remove();
    });
  }
  ...
  async _generateAccrualsUnified(contractId, start, end, freq, rateMode, fxConv, out){
    try{
      // 1) Base agenda (padrão) — usa engine original, se disponível; senão, usa nosso fallback.
      if (out.base){
      ...
      // 2) Recalculada + pagamentos (impacta montante do período)
      if (out.recalc){
        await this.createAccrualSheetRecalc(contractId, start, end, freq, rateMode, fxConv);
      }
      // 3) Histórico Completo (layout do cliente)
      if (out.hist){
        await this.createAccrualHistoricoCompleto(contractId, start, end, freq, rateMode, fxConv, 'MONTHLY');
      }
      // 4) Anotar pagamentos automaticamente
      if (out.autoAnnot){
        await this._annotateAllForContract(contractId);
      }
  ...
  // Fallback para base caso o método original não exista/esteja renomeado
  async _createAccrualBaseFallback(contractId, start, end, freq='Diário', rateMode='BASE', fxConv='DAILY'){
    // Implementa uma agenda de acúmulo simples sem pagamentos (compatível com colunas originais)
    const c = this._getContract(contractId);
    const wb = this.kernel.workbookManager.getActiveWorkbook();
    const isFX = String(c.moeda||c.currency||c.moedaorigem||'BRL').toUpperCase()!=='BRL';
    const ccy = c.moeda||c.currency||'USD';
    const name = `_Loan_Accrual_${contractId}_${start}_${end}_${freq}`;
    let sh = wb.sheets.find(s=> s.name===name) || wb.addSheet(name);
    sh.columns=[]; sh.rows=0;
    const H = isFX
    ...
    const startD = U.toD(start), endD = U.toD(end);
    const pts=[]; if (freq==='Diário'){ for(let d=U.addDays(startD,1); d<=endD; d=U.addDays(d,1)) pts.push(U.fmtD(d)); }
    else if (freq==='Anual'){ for (let d=U.addYear(startD,1); d<=endD; d=U.addYear(d,1)) pts.push(U.fmtD(d)); }
    else { for (let d=U.addMonth(startD,1); d<=endD; d=U.addMonth(d,1)) pts.push(U.fmtD(d)); }

    let balFX = isFX ? (c.principalorigem || c.principalOrigin || 0) : (c.principalbrl||0);
    let r=1, accBRL=0, prev=start;
    for (const dt of pts){
      const rEff = await this._legsEffTotal(c, prev, dt, rateMode);
      const ptax = await this._ptaxBCBByMode(dt, ccy, fxConv);
      const openingBRL = isFX ? U.round(balFX*ptax, 2, c.rounding||'HALF_UP') : balFX;
      const I_FX  = isFX ? U.round(balFX*rEff, 8, 'HALF_UP') : 0;
      const I_BRL = isFX ? U.round(I_FX*ptax, 2, c.rounding||'HALF_UP') : U.round(openingBRL*rEff, 2, c.rounding||'HALF_UP');
      const amtFX = isFX ? U.round(balFX + I_FX, 8, 'HALF_UP') : 0;
      const amtBRL= isFX ? U.round(amtFX * ptax, 2, c.rounding||'HALF_UP') : U.round(openingBRL + I_BRL, 2, c.rounding||'HALF_UP');
  ...
  // ---------- Recalc Schedule (+Payments) (unchanged behavior, but now used by unified) ----------
  async createAccrualSheetRecalc(contractId, start, end, freq='Diário', rateMode='BASE', fxConv='DAILY'){
    // (the same implementation as consolidated version; kept intact below)
    return await (this._createAccrualSheetRecalc_impl?.(contractId,start,end,freq,rateMode,fxConv) || this._createAccrualSheetRecalc_impl_fallback(contractId,start,end,freq,rateMode,fxConv));
  }

  // Store the fallback implementation here to ensure availability
  async _createAccrualSheetRecalc_impl_fallback(contractId, start, end, freq='Diário', rateMode='BASE', fxConv='DAILY'){
    const c = this._getContract(contractId);
    if (!c){ this.ui.showToast('Contrato não encontrado.','error'); return false; }
    const wb = this.kernel.workbookManager.getActiveWorkbook();
    const isFX = String(c.moeda||c.currency||c.moedaorigem||'BRL').toUpperCase()!=='BRL';
    const ccy = c.moeda||c.currency||'USD';
    const name = `_Loan_Accrual_Recalc_${contractId}_${start}_${end}_${freq}_${rateMode}_${fxConv}`;
    let sh = wb.sheets.find(s=> s.name===name) || wb.addSheet(name);
    sh.columns=[]; sh.rows=0;
    ...
    const ledgerName = `${LoanPluginV971.S.LEDGER}${contractId}`;
    const lsh = wb.sheets.find(s=> s.name===ledgerName);
    const paysByDate = new Map();
    const startD = U.toD(start), endD = U.toD(end);
    if (lsh && lsh.rows>1){
      const LH = this._buildHeaderMap(lsh);
      for (let r=1;r<lsh.rows;r++){
        const d = this._ISO(lsh.getCellValue(r, LH['data'] ?? 0)); if (!d) continue;
        ...
        paysByDate.set(d, {
          ...
          pI_fx : this._num(lsh.getCellValue(r, LH['juros pagos usd'] ?? LH['juros pagos '+(c.currency||'usd').toLowerCase()] ?? -1)),
          pP_fx : this._num(lsh.getCellValue(r, LH['amortização usd'] ?? LH['amortização '+(c.currency||'usd').toLowerCase()] ?? -1))
        });
      }
    }
    ...
    let balFX = isFX ? (c.principalorigem || c.principalOrigin || 0) : (c.principalbrl||0);
    // subtract pre-start amortizations in FX
    if (isFX && lsh && lsh.rows>1){
      const LH = this._buildHeaderMap(lsh);
      for (let r=1;r<lsh.rows;r++){
        const d = this._ISO(lsh.getCellValue(r, LH['data'] ?? 0));
        if (!d || U.toD(d) < startD) {
          const pfx = this._num(lsh.getCellValue(r, LH['amortização '+(ccy||'USD').toLowerCase()] ?? -1));
          if (pfx) balFX = U.round(balFX - pfx, 8, 'HALF_UP');
        }
      }
    }

    let r=1, accFX=0, accBRL=0, prev=start;
    for (const dt of periodEnds){
      const rEff = await this._legsEffTotal(c, prev, dt, rateMode);
      const ptax = await this._ptaxBCBByMode(dt, ccy, fxConv);
      const openingBRL = isFX ? U.round(balFX * ptax, 2, c.rounding||'HALF_UP') : balFX;
      const I_FX  = isFX ? U.round(balFX * rEff, 8, 'HALF_UP') : 0;
      const I_BRL = isFX ? U.round(I_FX * ptax, 2, c.rounding||'HALF_UP') : U.round(openingBRL * rEff, 2, c.rounding||'HALF_UP');
      let amtFX   = isFX ? U.round(balFX + I_FX, 8, 'HALF_UP') : 0;
      let amtBRL  = isFX ? U.round(amtFX * ptax, 2, c.rounding||'HALF_UP') : U.round(openingBRL + I_BRL, 2, c.rounding||'HALF_UP');

      accFX  = isFX ? U.round(accFX + I_FX, 8, 'HALF_UP') : accFX;
      accBRL = U.round(accBRL + I_BRL, 2, c.rounding||'HALF_UP');

      let pI_brl=0, pP_brl=0, pI_fx=0, pP_fx=0, event='';
      const pay = paysByDate.get(dt);
      if (pay){
  ...
  // ---------- Histórico Completo (ex-Santander) ----------
  async createAccrualHistoricoCompleto(contractId, start, end, freq='Diário', rateMode='BASE', fxConv='DAILY', groupMode='MONTHLY'){
    return await this.createAccrualSantander?.(contractId, start, end, freq, rateMode, fxConv, groupMode)
        || await this._createAccrualHistoricoCompleto_fallback(contractId, start, end, freq, rateMode, fxConv, groupMode);
  }
  async _createAccrualHistoricoCompleto_fallback(contractId, start, end, freq='Diário', rateMode='BASE', fxConv='DAILY', groupMode='MONTHLY'){
    // reuse the previous injected implementation with exactly the requested headers
    const c = this._getContract(contractId);
    if (!c){ this.ui.showToast('Contrato não encontrado.','error'); return false; }
    const wb = this.kernel.workbookManager.getActiveWorkbook();
    const isFX = String(c.moeda||c.currency||c.moedaorigem||'BRL').toUpperCase()!=='BRL';
    const ccy = c.moeda||c.currency||'USD';
    const name = `_Loan_Accrual_Report_${contractId}_${start}_${end}`;
    let sh = wb.sheets.find(s=> s.name===name) || wb.addSheet(name);
    sh.columns=[]; sh.rows=0;
    const H = ['DATE','INTEREST RATE','BACEN TAX','PRIOR BALANCE','INTEREST USD DAY','INTEREST USD','INTEREST USD acum','INTEREST BRL (BACEN)','INTEREST BRL (BACEN) acum','INTEREST BRL (group) acum','VC','IOF','IR','PAYMENT OF INTERESTS USD','PAYMENT OF PRINCIPAL USD','PAYMENT OF INTERESTS BRL','PAYMENT OF PRINCIPAL BRL','DAYS'];
    H.forEach((h,i)=> this._setCell(sh,0,i,h,'HEADER'));
    const iofPct = parseFloat(c['IOF %'] ?? c['iof %'] ?? 0) || 0;
    const irPct  = parseFloat(c['IR %']  ?? c['ir %']  ?? 0) || 0;
    const irMode = String(c['IR Mode']  ?? c['ir mode'] ?? 'NONE').toUpperCase();
    const group  = String(c['Group Mode'] ?? c['group mode'] ?? groupMode).toUpperCase();
    const ledgerName = `${LoanPluginV971.S.LEDGER}${contractId}`;
    const lsh = wb.sheets.find(s=> s.name===ledgerName);
    const paysByDate = new Map();
    const startD = U.toD(start), endD = U.toD(end);
    if (lsh && lsh.rows>1){
      ...
      for (let r=1;r<lsh.rows;r++){
        ...
        paysByDate.set(d, {
          ...
          I_fx : this._num(lsh.getCellValue(r, LH['juros pagos usd'] ?? LH['juros pagos '+(c.currency||'usd').toLowerCase()] ?? -1)),
          P_fx : this._num(lsh.getCellValue(r, LH['amortização usd'] ?? LH['amortização '+(c.currency||'usd').toLowerCase()] ?? -1))
        });
      }
    }
    const ptaxCtr = await this._getContractPTAXFixed(c, start);
    const periodEnds=[]; if (freq==='Diário'){ for (let d=U.addDays(startD,1); d<=endD; d=U.addDays(d,1)) periodEnds.push(U.fmtD(d)); }
    else if (freq==='Anual'){ for (let d=U.addYear(startD,1); d<=endD; d=U.addYear(d,1)) periodEnds.push(U.fmtD(d)); }
    else { for (let d=U.addMonth(startD,1); d<=endD; d=U.addMonth(d,1)) periodEnds.push(U.fmtD(d)); }
    ...
    if (isFX && lsh && lsh.rows>1){
      const LH = this._buildHeaderMap(lsh);
      for (let r=1;r<lsh.rows;r++){
        const d = this._ISO(lsh.getCellValue(r, LH['data'] ?? 0)); if (!d || U.toD(d) >= startD) continue;
        const pfx = this._num(lsh.getCellValue(r, LH['amortização '+(ccy||'USD').toLowerCase()] ?? -1));
        if (pfx) balFX = U.round(balFX - pfx, 8, 'HALF_UP');
      }
    }

    let r=1, accFX=0, accBRL=0, groupAcc=0, prev=start, runUSD=0;
    let lastMonth = (new Date(start)).getMonth();
    for (const dt of periodEnds){
      const dtDate = new Date(dt); const month = dtDate.getMonth();
      if (group==='MONTHLY' && month!=lastMonth){ groupAcc = 0; lastMonth = month; }
      const rEff = await this._legsEffTotal(c, prev, dt, rateMode);
      const ptax  = await this._ptaxBCBByMode(dt, ccy, fxConv);
      const I_FX  = isFX ? U.round(balFX * rEff, 8, 'HALF_UP') : 0;
      const I_BRL = isFX ? U.round(I_FX * ptax, 2, c.rounding||'HALF_UP') : U.round((balFX) * rEff, 2, c.rounding||'HALF_UP');
      const amtFX   = isFX ? U.round(balFX + I_FX, 8, 'HALF_UP') : 0;
      const amtBRL  = isFX ? U.round(amtFX * ptax, 2, c.rounding||'HALF_UP') : U.round((balFX) + I_BRL, 2, c.rounding||'HALF_UP');
      runUSD = isFX ? U.round(runUSD + I_FX, 8, 'HALF_UP') : 0;
      const usdAcum = isFX ? amtFX : 0;

      accFX  = isFX ? U.round(accFX + I_FX, 8, 'HALF_UP') : accFX;
      accBRL = U.round(accBRL + I_BRL, 2, c.rounding||'HALF_UP');
      groupAcc = U.round(groupAcc + I_BRL, 2, c.rounding||'HALF_UP');
      const VC = isFX ? U.round(balFX * (ptax - ptaxCtr), 2, c.rounding||'HALF_UP') : 0;
      const IOF = U.round(I_BRL * (iofPct||0), 2, c.rounding||'HALF_UP');
      const IR  = (irMode==='DAILY') ? U.round(I_BRL * (irPct||0), 2, c.rounding||'HALF_UP')
                 : (irMode==='MONTHLY' ? (0) : 0);

      let pI_brl=0, pP_brl=0, pI_fx=0, pP_fx=0;
      const pay = paysByDate.get(dt);
      if (pay){
      ...
      const days = U.diffDays(prev, dt);
      let cidx=0;
      this._setCell(sh, r, cidx++, dt, 'DATE');
      this._setCell(sh, r, cidx++, rEff, 'PCT4');
      this._setCell(sh, r, cidx++, await this._ptaxBCBByMode(dt, ccy, fxConv), 'FX');
      this._setCell(sh, r, cidx++, isFX?balFX:balFX, isFX?`CURFX:${ccy}`:'CUR');
  ...
  // ---------- Automatic payment annotation & interest sync ----------
  _autoAnnotateInit(){
    ...
    const hash = () => {
      ...
      for (let i=1;i<cs.rows;i++){ const id = String(cs.getCellValue(i,0)||''); if (!id) continue;
        const lsh = wb.sheets.find(s=> s.name===`${LoanPluginV971.S.LEDGER}${id}`);
        if (!lsh) continue;
        out.push(id, lsh.rows, lsh.columns?.length||0);
        if (lsh.rows>1){ out.push(String(lsh.getCellValue(lsh.rows-1,0)||'')); }
      }
      return out.join('|');
    };
    let last = hash();
    this.__annotInterval = setInterval(()=>{
      const cur = hash();
      if (cur!==last){ last = cur;
        try{
          const cs = wb?.sheets.find(s=> s.name===LoanPluginV971.S.CONTRACTS);
          if (!cs || cs.rows<=1) return;
          for (let i=1;i<cs.rows;i++){
            const id = String(cs.getCellValue(i,0)||''); if (!id) continue;
            this._annotateAllForContract(id);
            this.
  _syncLedgerInterestsForContract(contractId){
    try{
      const wb = this.kernel.workbookManager.getActiveWorkbook();
      const cs = wb?.sheets.find(s=> s.name===LoanPluginV971.S.CONTRACTS);
      if (!cs || cs.rows<=1) return;
      const c = this._getContract(contractId); if (!c) return;
      const ccy = c.moeda||c.currency||'USD'; const isFX = String(ccy||'BRL').toUpperCase()!=='BRL';
      const lsh = wb.sheets.find(s=> s.name===`${LoanPluginV971.S.LEDGER}${contractId}`);
      if (!lsh) return;
      const LH = this._buildHeaderMap(lsh);
      const colI_BRL = this._ensureHeader(lsh, 'juros período brl');
      const colI_FX  = isFX ? this._ensureHeader(lsh, `juros período ${ccy}`) : -1;

      // Gather ledger rows sorted by date
      const rows=[];
      for (let r=1;r<lsh.rows;r++){
        const d = this._ISO(lsh.getCellValue(r, LH['data'] ?? 0)); if (!d) continue;
        rows.push({r, d});
      ...
      // Run balance from contract start to each ledger date, compounding daily like accrual
      const start = String(c.startDate||c.inicio||c['Data Início']||rows[0]?.d||new Date().toISOString().slice(0,10));
      let prev = start;
      let balFX = isFX ? (c.principalorigem || c.principalOrigin || 0) : (c.principalbrl||0);

      for (const it of rows){
        // accrue from prev to it.d (daily steps)
        for (let day = U.addDays(U.toD(prev), 1); day <= U.toD(it.d); day = U.addDays(day, 1)){
          const day0 = U.fmtD(U.addDays(day, -1)), day1 = U.fmtD(day);
          const rEff = this._legsEffTotalSync?.(c, day0, day1, 'BASE') ?? (this._legsEffTotal ? (await this._legsEffTotal(c, day0, day1, 'BASE')) : 0);
          const I_FX  = isFX ? U.round(balFX * rEff, 8, 'HALF_UP') : 0;
          const ptax  = await this._ptaxBCBByMode(day1, ccy, 'DAILY');
          const I_BRL = isFX ? U.round(I_FX * ptax, 2, c.rounding||'HALF_UP') : U.round(balFX * rEff, 2, c.rounding||'HALF_UP');
          // If this day equals the ledger row date, write the period interest
          if (day1 === it.d){
            this._setCell(lsh, it.r, colI_BRL, I_BRL, 'CUR');
            if (isFX && colI_FX>=0) this._setCell(lsh, it.r, colI_FX, I_FX, `CURFX:${ccy}`);
          }
          // compound for next day
          if (isFX) balFX = U.round(balFX + I_FX, 8, 'HALF_UP');
          else      balFX = U.round(balFX + I_BRL, 2, c.rounding||'HALF_UP');

          // apply amortizations if exist on this day
          if (day1 === it.d){
            const pfx = isFX ? (this._num(lsh.getCellValue(it.r, LH['amortização '+(ccy||'usd').toLowerCase()] ?? -1)) || 0) : 0;
            const pbrl = this._num(lsh.getCellValue(it.r, LH['amortização brl'] ?? -1)) || 0;
  ...
  async _annotateAllForContract(contractId){
    ...
    if (!cs || cs.rows<=1) return;
    let ccy='USD';
    for (let r=1;r<cs.rows;r++){ if (String(cs.getCellValue(r,0)||'')===contractId){ ccy=String(cs.getCellValue(r,4)||'USD'); break; } }
    const isFX = String(ccy||'BRL').toUpperCase()!=='BRL';
    const lsh = wb.sheets.find(s=> s.name===`${LoanPluginV971.S.LEDGER}${contractId}`);
    if (!lsh) return;
    const LH = this._buildHeaderMap(lsh);
    for (let r=1;r<lsh.rows;r++){
      const d = this._ISO(lsh.getCellValue(r, LH['data'] ?? 0)); if (!d) continue;
      const payI_BRL = this._num(lsh.getCellValue(r, LH['juros pagos brl'] ?? -1));
      const payP_BRL = this._num(lsh.getCellValue(r, LH['amortização brl'] ?? -1));
      const payI_FX  = isFX ? this._num(lsh.getCellValue(r, LH['juros pagos '+(ccy||'usd').toLowerCase()] ?? -1)) : 0;
      const payP_FX  = isFX ? this._num(lsh.getCellValue(r, LH['amortização '+(ccy||'usd').toLowerCase()] ?? -1)) : 0;
      this._annotateAccruals(contractId, d, { isFX, ccy, payI_BRL, payP_BRL, payI_FX, payP_FX });
    }
    this.kernel.ui.recalculateAll?.(true); this.kernel.ui.renderGrid?.();
  }

  // Enforce: ledger "juros do período" equals accrual engine
  _syncLedgerInterestsForContract(contractId){
    try{
      const wb = this.kernel.workbookManager.getActiveWorkbook();
      const cs = wb?.sheets.find(s=> s.name===LoanPluginV971.S.CONTRACTS);
      if (!cs || cs.rows<=1) return;
      let c=null; for (let r=1;r<cs.rows;r++){ if (String(cs.getCellValue(r,0)||'')===contractId){ c = this._getContract(contractId); break; } }
      if (!c) return;
      const ccy = c.moeda||c.currency||'USD'; const isFX = String(ccy||'BRL').toUpperCase()!=='BRL';
      const lsh = wb.sheets.find(s=> s.name===`${LoanPluginV971.S.LEDGER}${contractId}`);
      if (!lsh) return;
      ...
      for (let r=1;r<lsh.rows;r++){
        const d = this._ISO(lsh.getCellValue(r, LH['data'] ?? 0)); if (!d) continue;
        const prev = U.fmtD(U.addDays(U.toD(d), -1));
        const rEff = this._legsEffTotalSync?.(c, prev, d, 'BASE') || 0; // try sync variant if available
        const ptax = this._ptaxBCBByModeSync?.(d, ccy, 'DAILY') || 0;
        const balFX = isFX ? (c.principalorigem || c.principalOrigin || 0) : (c.principalbrl||0); // approximation; for exact, would track carried balance
        const I_FX  = isFX ? U.round(balFX * rEff, 8, 'HALF_UP') : 0;
        const I_BRL = isFX ? U.round(I_FX * ptax, 2, c.rounding||'HALF_UP') : U.round(balFX * rEff, 2, c.rounding||'HALF_UP');
        this._setCell(lsh, r, colI_BRL, I_BRL, 'CUR');
        if (isFX && colI_FX>=0) this._setCell(lsh, r, colI_FX, I_FX, `CURFX:${ccy}`);
      }
    }catch(e){ this.warn('sync-ledger-interest', e); }
  }
  ...
  _wizardExport(){
    ...
    if (!cs || cs.rows<=1) return this.ui.showToast('Nenhum contrato.','warning');
    const ids=[]; for (let i=1;i<cs.rows;i++) ids.push(String(cs.getCellValue(i,0)||''));
    const m = this.ui.createModal('📤 Exportar planilha(s)');
    m.body.innerHTML = `
      <div style="display:grid;gap:12px">
        <div class="grp"><label>Contrato</label><select id="cid" class="formula-input">${ids.map(x=>`<option>${x}</option>`).join('')}</select></div>
    ...
    f.querySelector('.btn-ok').addEventListener('click', async()=>{
      const id = String(m.body.querySelector('#cid').value);
      const scopeAll = m.body.querySelector('#scope_all').checked;
      if (!scopeAll){ this.exportActiveSheetXLSX?.(); }
      else { this._exportAllSheetsCSVForContract(id); }
      m.overlay.remove();
    });
  }
  ...
  _exportAllSheetsCSVForContract(contractId){
    const wb = this.kernel.workbookManager.getActiveWorkbook();
    const sheets = wb.sheets.filter(s=>{
      const n = String(s.name||'');
      return n.includes(`_${contractId}_`) || n.endsWith(contractId) || n.includes(`_${contractId}`);
    });
    sheets.forEach(sh=>{
      const csv = this._toCSV(sh);
      const blob = new Blob([csv], {type:'text/csv;charset=utf-8;'});
  ...
  _toCSV(sh){
    const rows = sh.rows||0; const cols = sh.columns?.length||0;
    const esc = (v)=>{
      if (v==null) return '';
    ...
    const out=[];
    // header row
    out.push(Array.from({length:cols}, (_,c)=> esc(sh.columns[c]?.name||'')).join(';'));
    for (let r=1;r<rows;r++){
      out.push(Array.from({length:cols}, (_,c)=> esc(sh.getCellValue(r,c))).join(';'));
    }
    return out.join('\n');
  }
