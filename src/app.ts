...
import { VirtualGrid } from './@ui/grid-virtual-consolidated';
...
if (typeof window !== 'undefined') {
  ...
  private async initUI(): Promise<void> {
    ...
    // Simple UI for now - will be replaced with full UI shell
    const company = kernel.companyManager.getActiveCompany();
    const workbooks = kernel.workbookManager.listWorkbooks();
      ...
      <div class="app-container">
        ...
        <header class="app-header">
          <div class="header-left">
            ...
            <span class="company-badge">${company?.name || 'Sem empresa'}</span>
          </div>
          <div class="header-right">
            <button id="btn-new-workbook" class="btn btn-primary">
              📄 Novo Workbook
            </button>
            <button id="btn-settings" class="btn">⚙️</button>
          </div>
        ...
        <div class="app-content">
          ...
          <aside class="sidebar">
            ...
            <div id="workbook-list" class="workbook-list">
              ${workbooks.length === 0 
                ? '<p class="empty-state">Nenhum workbook criado</p>'
                : workbooks.map(wb => `
                    <div class="workbook-item ${wb.id === kernel.workbookManager.getActiveWorkbook()?.id ? 'active' : ''}"
                         data-workbook-id="${wb.id}">
                      <span class="workbook-name">${wb.name}</span>
          ...
          <main class="grid-container">
            <div class="grid-placeholder">
              <div class="placeholder-content">
                <h2>📊 Grid Virtual</h2>
                <p>Selecione ou crie um workbook para começar</p>
                <p class="placeholder-info">
                  O grid virtualizado será implementado na Fase 4<br>
                  Por enquanto, teste a criação de workbooks e fórmulas via console
                </p>
                <div class="console-examples">
                  <h4>Exemplos no Console:</h4>
                  <code>
                    const wb = DJKernel.createWorkbook("Teste");<br>
                    const sheet = wb.getActiveSheet();<br>
                    sheet.setCell(0, 0, "Nome");<br>
                    sheet.setCell(0, 1, "Valor");<br>
                    sheet.setCell(1, 0, "Item 1");<br>
                    sheet.setCell(1, 1, 100);<br>
                    sheet.setCell(2, 1, "=B2*2", { formula: "=B2*2", type: "formula" });<br>
                    await DJKernel.recalculate(sheet.id);<br>
                    sheet.getCellValue(2, 1); // 200
                  </code>
                </div>
              </div>
            </div>
        ...
        <!-- Status Bar -->
        <footer class="status-bar">
          <span class="status-info">✅ Pronto | v${kernel.version}</span>
          <span class="status-company">Empresa: ${company?.name || 'N/A'}</span>
          <span class="status-workbook">Workbooks: ${workbooks.length}</span>
        </footer>
      </div>
  ...
  private setupEventListeners(): void {
    // New workbook button
    const btnNew = document.getElementById('btn-new-workbook');
    ...
    btnSettings?.addEventListener('click', () => {
      alert("Settings (TODO: implementar painel de configurações)");
    });
    
    // Workbook list items
    document.querySelectorAll('.workbook-item').forEach(item => {
      item.addEventListener('click', () => {
        const workbookId = item.getAttribute('data-workbook-id');
        if (workbookId) {
          kernel.workbookManager.setActiveWorkbook(workbookId);
          kernel.sessionManager.setActiveWorkbook(workbookId);
          this.refreshUI();
  ...
  private setupKernelEventListeners(): void {
    ...
    // Listen to recalc done
    kernel.eventBus.on('kernel:recalc-done', (payload: any) => {
      logger.info(`[App] Recálculo concluído em ${payload.duration.toFixed(2)}ms`);
      this.setupConsoleLogger();
    });
  }
  
  private refreshUI(): void {
    // Simple refresh - re-render the UI
    this.initUI();
  }
